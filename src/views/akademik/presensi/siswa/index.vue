<template>
  <div style="padding:30px;">
    <el-alert :closable="false" title="List Presensi Siswa" />
    ...
    >
      ...
      <el-table-column label="Materi">
        <template slot-scope="scope">{{ scope.row.jadwal.materi.nama_materi }}</template>
      </el-table-column>
      <el-table-column label="Nama Siswa">
        <template slot-scope="scope">{{ scope.row.user.nama }}</template>
      </el-table-column>
      <el-table-column label="Jam Presensi">
        <template slot-scope="scope">{{ scope.row.jam_absen }}</template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getMateri() {
      axios
        .get(process.env.VUE_APP_BASE_API + "/materi", { headers: this.auth })
        .then(response => {
